...
class NewScalaCliProjectWizard_ScalaWithScalaCLI extends NewScalaCliProjectWizardTestBase {
  ...
  def testCreateProjectUsingScalaWithScalaCLI(scalaVersion: ScalaVersion): Unit = {
    ...
    runSimpleCreateProjectTest(SupportedScala3Version, expectedSemanticDbVersion = expectedSemanticDbVersion, useIndentationBasedSyntax = false, shouldExcludeScalaBuild = shouldExcludeScalaBuild)
  }
}
