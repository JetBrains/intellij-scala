...
trait Rule[-In, +Out, +A, +X] extends (In => Result[Out, A, X]) { 
  ...
  def as(name : String): Rule[In, Out, A, X] with Name = ruleWithName(name, this)
  
  def flatMap[Out2, B, X2 >: X](fa2ruleb : A => Out => Result[Out2, B, X2]): Rule[In, Out2, B, X2] = mapResult {
      case Success(out, a) => fa2ruleb(a)(out)
      case Failure => Failure
      case err @ Error(_) => err
  }
  
  def map[B](fa2b : A => B): Rule[In, Out, B, X] = flatMap { a => out => Success(out, fa2b(a)) }

  def filter(f : A => Boolean): Rule[In, Out, A, X] = flatMap { a => out => if(f(a)) Success(out, a) else Failure }

  def mapResult[Out2, B, Y](f : Result[Out, A, X] => Result[Out2, B, Y]): Rule[In, Out2, B, Y] = rule {
    in : In => f(apply(in))
  }
  ...
  def orError[In2 <: In]: Rule[In2, Out, A, Any] = this orElse(error[In2])

  def |[In2 <: In, Out2 >: Out, A2 >: A, X2 >: X](other : => Rule[In2, Out2, A2, X2]): Rule[In2, Out2, A2, X2] = orElse(other)

  def ^^[B](fa2b : A => B): Rule[In, Out, B, X] = map(fa2b)
  
  def ^^?[B](pf : PartialFunction[A, B]): Rule[In, Out, B, X] = filter (pf.isDefinedAt(_)) ^^ pf
  ...
  def ~-[Out2, B, X2 >: X](next : => Rule[Out, Out2, B, X2]): Rule[In, Out2, A, X2] = for (a <- this; b <- next) yield a

  def -~[Out2, B, X2 >: X](next : => Rule[Out, Out2, B, X2]): Rule[In, Out2, B, X2] = for (a <- this; b <- next) yield b
  ...
  def ~>[Out2, B, X2 >: X](next : => Rule[Out, Out2, A => B, X2]): Rule[In, Out2, B, X2] = for (a <- this; fa2b <- next) yield fa2b(a)
  
  /** Apply the result of this rule to the function returned by the previous rule */
  def <~:[InPrev, B, X2 >: X](prev : => Rule[InPrev, In, A => B, X2]): Rule[InPrev, Out, B, X2] = for (fa2b <- prev; a <- this) yield fa2b(a)
    
  def ~![Out2, B, X2 >: X](next : => Rule[Out, Out2, B, X2]): Rule[In, Out2, ~[A, B], Any] = for (a <- this; b <- next orError) yield new ~(a, b)
  
  def ~-![Out2, B, X2 >: X](next : => Rule[Out, Out2, B, X2]): Rule[In, Out2, A, Any] = for (a <- this; b <- next orError) yield a
  
  def -~![Out2, B, X2 >: X](next : => Rule[Out, Out2, B, X2]): Rule[In, Out2, B, Any] = for (a <- this; b <- next orError) yield b
  
  def -[In2 <: In](exclude : => Rule[In2, Any, Any, Any]): Rule[In2, Out, A, X] = !exclude -~ this
      
  /** ^~^(f) is equivalent to ^^ { case b1 ~ b2 => f(b1, b2) } 
   */
  def ^~^[B1, B2, B >: A <% B1 ~ B2, C](f : (B1, B2) => C): Rule[In, Out, C, X] = map { a =>
    (a : B1 ~ B2) match { case b1 ~ b2 => f(b1, b2) } 
  }
    
  /** ^~~^(f) is equivalent to ^^ { case b1 ~ b2 ~ b3 => f(b1, b2, b3) } 
   */
  def ^~~^[B1, B2, B3, B >: A <% B1 ~ B2 ~ B3, C](f : (B1, B2, B3) => C): Rule[In, Out, C, X] = map { a =>
    (a : B1 ~ B2 ~ B3) match { case b1 ~ b2 ~ b3 => f(b1, b2, b3) } 
  }
    
  /** ^~~~^(f) is equivalent to ^^ { case b1 ~ b2 ~ b3 ~ b4 => f(b1, b2, b3, b4) } 
   */
  def ^~~~^[B1, B2, B3, B4, B >: A <% B1 ~ B2 ~ B3 ~ B4, C](f : (B1, B2, B3, B4) => C): Rule[In, Out, C, X] = map { a =>
    (a : B1 ~ B2 ~ B3 ~ B4) match { case b1 ~ b2 ~ b3 ~ b4 => f(b1, b2, b3, b4) } 
  }
     
  /** ^~~~~^(f) is equivalent to ^^ { case b1 ~ b2 ~ b3 ~ b4 ~ b5 => f(b1, b2, b3, b4, b5) } 
   */
  def ^~~~~^[B1, B2, B3, B4, B5, B >: A <% B1 ~ B2 ~ B3 ~ B4 ~ B5, C](f : (B1, B2, B3, B4, B5) => C): Rule[In, Out, C, X] = map { a =>
    (a : B1 ~ B2 ~ B3 ~ B4 ~ B5) match { case b1 ~ b2 ~ b3 ~ b4 ~ b5 => f(b1, b2, b3, b4, b5) } 
  }
      
  /** ^~~~~~^(f) is equivalent to ^^ { case b1 ~ b2 ~ b3 ~ b4 ~ b5 ~ b6 => f(b1, b2, b3, b4, b5, b6) } 
   */
  def ^~~~~~^[B1, B2, B3, B4, B5, B6, B >: A <% B1 ~ B2 ~ B3 ~ B4 ~ B5 ~ B6, C](f : (B1, B2, B3, B4, B5, B6) => C): Rule[In, Out, C, X] = map { a =>
    (a : B1 ~ B2 ~ B3 ~ B4 ~ B5 ~ B6) match { case b1 ~ b2 ~ b3 ~ b4 ~ b5 ~ b6 => f(b1, b2, b3, b4, b5, b6) } 
  }
       
  /** ^~~~~~~^(f) is equivalent to ^^ { case b1 ~ b2 ~ b3 ~ b4 ~ b5 ~ b6 => f(b1, b2, b3, b4, b5, b6) } 
   */
  def ^~~~~~~^[B1, B2, B3, B4, B5, B6, B7, B >: A <% B1 ~ B2 ~ B3 ~ B4 ~ B5 ~ B6 ~ B7, C](f : (B1, B2, B3, B4, B5, B6, B7) => C): Rule[In, Out, C, X] = map { a =>
    (a : B1 ~ B2 ~ B3 ~ B4 ~ B5 ~ B6 ~ B7) match { case b1 ~ b2 ~ b3 ~ b4 ~ b5 ~ b6 ~b7 => f(b1, b2, b3, b4, b5, b6, b7) } 
  }
   
  /** >~>(f) is equivalent to >> { case b1 ~ b2 => f(b1, b2) } 
   */
  def >~>[Out2, B1, B2, B >: A <% B1 ~ B2, C, X2 >: X](f : (B1, B2) => Out => Result[Out2, C, X2]): Rule[In, Out2, C, X2] = flatMap { a =>
    (a : B1 ~ B2) match { case b1 ~ b2 => f(b1, b2) } 
  }

  /** ^-^(f) is equivalent to ^^ { b2 => b1 => f(b1, b2) } 
   */
  def ^-^ [B1, B2 >: A, C](f : (B1, B2) => C): Rule[In, Out, (B1) => C, X] = map { b2 : B2 => b1 : B1 => f(b1, b2) }
    
 /** ^~>~^(f) is equivalent to ^^ { case b2 ~ b3 => b1 => f(b1, b2, b3) } 
  */
 def ^~>~^ [B1, B2, B3, B >: A <% B2 ~ B3, C](f : (B1, B2, B3) => C): Rule[In, Out, (B1) => C, X] = map { a =>
