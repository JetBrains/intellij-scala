...
class NewSbtProjectWizardFullImportTest extends NewSbtProjectWizardTestBase with ExactMatch {
  ...
  def testCreateSbt_0_13_Project(): Unit =
    runProjectStructureOnlyTest(sbt013ProjectConfig, useCoursier = false)

  def testCreateSbt_1_0_Project(): Unit =
    runProjectStructureOnlyTest(sbt10ProjectConfig, useCoursier = false)
  ...
  private def runProjectStructureOnlyTest(config: SbtWizardProjectConfig, useCoursier: Boolean = true): Unit = {
