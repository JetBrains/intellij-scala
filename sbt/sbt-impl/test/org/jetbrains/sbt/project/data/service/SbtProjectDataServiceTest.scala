...
class SbtProjectDataServiceTest extends ProjectDataServiceTestCase {
  ...
  def testIncrementalityTypeForSharedModules(): Unit = {
    val testProject = new project {
      ...
      arbitraryNodes += new SbtProjectNode(SbtProjectData(None, "", getProject.getBasePath, prodTestSourcesSeparated = false))
    }.build.toDataNode
  ...
  private def generateProject(
    ...
    new project {
      ...
      arbitraryNodes += new SbtProjectNode(SbtProjectData(jdk, sbtVersion, getProject.getBasePath, prodTestSourcesSeparated = false))
    }.build.toDataNode
