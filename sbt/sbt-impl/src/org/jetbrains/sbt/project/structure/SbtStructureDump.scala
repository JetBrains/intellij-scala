...
class SbtStructureDump {
  ...
  def dumpFromShell(
    ...
    val aggregator = SbtShellCommunication.messageAggregatorWithReporter(
      shell,
      reporter,
      EventId(s"dump:${UUID.randomUUID()}"),
      optProcessOutputBuilder,
      SbtBundle.message("sbt.extracting.project.structure.from.sbt.shell"),
      SbtBundle.message("sbt.project.structure.extracted")
    )
