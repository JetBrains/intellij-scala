...
object SbtModuleBuilderUtil {
  ...
  def doSetupModule(module: Module, externalProjectSettings: SbtProjectSettings, @Nullable contentEntryPath: String): Unit = {
    Option(contentEntryPath).foreach(tryToSetupModule(module, externalProjectSettings, _))
  }

  def tryToSetupModule(module: Module, externalProjectSettings: SbtProjectSettings, contentEntryPath: String): Unit = {
    val dir = getOrCreateDir(contentEntryPath)
    dir.foreach(doSetupModule(module, externalProjectSettings, _))
  }

  def doSetupModule(module: Module, externalProjectSettings: SbtProjectSettings, contentRootDir: File): Unit = {
