 ...
 * After the process terminates, the test waits until `DebuggerManagerEx` reports no remaining sessions and only then
 * allows normal fixture teardown to continue.
 */
