...
object LocalRepoPackager extends AutoPlugin {
  ...
  private def scalaBinaryVersionForSbtVersion(sbtVersion: String): String = {
    if (sbtVersion.startsWith("0"))
      "2.10"
    else if (sbtVersion.startsWith("1"))
      "2.12"
    else if (sbtVersion.startsWith("2"))
      "3"
    else
      throw new IllegalArgumentException(s"unsupported sbt version: $sbtVersion")
  }
