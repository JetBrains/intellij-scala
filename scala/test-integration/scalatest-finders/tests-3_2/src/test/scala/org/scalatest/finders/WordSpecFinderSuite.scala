...
class WordSpecFinderSuite extends FinderSuite {
  ...
  test("WordSpecFinder should find test name for tests written in test suite that extends org.scalatest.FeatureSpec, using should, which and taggedAs") {
    class WordSpecClassName extends AnyWordSpec {
      "tagged" should {
        "not be tagged" in {}
    ...
    val suiteClass = classOf[WordSpecClassName]
    val finders = LocationUtils.getFinders(suiteClass)
    assert(finders.size == 1, "org.scalatest.WordSpec should have 1 finder, but we got: " + finders.size)
    val finder = finders.get(0)
    assert(finder.getClass == classOf[WordSpecFinder], "Suite that uses org.scalatest.WordSpec should use WordSpecFinder.")
    ...
    val classDef           = new ClassDefinition(suiteClass.getName, null, Array.empty, "WordSpecClassName")
    val constructorBlock   = new ConstructorBlock(suiteClass.getName, classDef, Array.empty)
    val shouldNode         = invocation(toStringTarget("tagged"), constructorBlock, "should")
    val notBeTaggedStrNode = toStringTarget("not be tagged")
    val notTaggedInNode    = invocation(notBeTaggedStrNode, shouldNode, "in")
    val beTaggedStrNode    = toStringTarget("be tagged")
    val taggedAsNode       = invocation(beTaggedStrNode, null, "taggedAs")
    val taggedInNode       = invocation(taggedAsNode, shouldNode, "in")

    taggedAsNode.injectParent(taggedInNode)
    notBeTaggedStrNode.injectParent(notTaggedInNode)
    beTaggedStrNode.injectParent(taggedAsNode)

    expectSelection(finder.find(shouldNode), suiteClass.getName, "tagged", Array("tagged should not be tagged", "tagged should be tagged"))
    expectSelection(finder.find(notBeTaggedStrNode), suiteClass.getName, "tagged should not be tagged", Array("tagged should not be tagged"))
    expectSelection(finder.find(notTaggedInNode), suiteClass.getName, "tagged should not be tagged", Array("tagged should not be tagged"))

    expectSelection(finder.find(beTaggedStrNode), suiteClass.getName, "tagged should be tagged", Array("tagged should be tagged"))
    expectSelection(finder.find(taggedAsNode), suiteClass.getName, "tagged should be tagged", Array("tagged should be tagged"))
    expectSelection(finder.find(taggedInNode), suiteClass.getName, "tagged should be tagged", Array("tagged should be tagged"))
  }
