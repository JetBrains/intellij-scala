...
class AllInPackageTestData(config: AbstractTestRunConfiguration) extends TestConfigurationData(config) {
  ...
  override def checkSuiteAndTestName: CheckResult =
    for {
      ...
      _ <- check(pack != null, configurationException(TestingSupportBundle.message("test.config.package.does.not.exist")))
    } yield ()
  ...
  private def findTestSuites(scope: GlobalSearchScope): Seq[String] = {
    ...
    if (pack == null) throw executionException(TestingSupportBundle.message("test.run.config.test.package.not.found", testPackagePath))
    ...
    if (classes.isEmpty)
      throw executionException(TestingSupportBundle.message("test.config.did.not.find.suite.classes.in.package", pack.getQualifiedName))
