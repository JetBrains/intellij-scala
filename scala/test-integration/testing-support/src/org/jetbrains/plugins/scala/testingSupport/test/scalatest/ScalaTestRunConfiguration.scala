        ...
        con match {
          case owner: ScModifierListOwner =>
            if (owner.hasModifierProperty(PsiModifier.PUBLIC)) {
              ...
              val psiManager = ScalaPsiManager.instance(clazz.getProject)
              val mapPsiClass = psiManager.getCachedClass(ProjectScope.getAllScope(clazz.getProject), "scala.collection.immutable.Map").orNull
              val mapClass = ScalaType.designator(mapPsiClass)
              val paramClass = firstParam.getType.toScType()(clazz.getProject)
