...
class ClassTestData(config: AbstractTestRunConfiguration) extends TestConfigurationData(config) {
  ...
  override def checkSuiteAndTestName: CheckResult =
    for {
      _ <- checkModule
      _ <- check(StringUtils.isNotBlank(testClassPath), configurationException(TestingSupportBundle.message("test.config.test.class.is.not.specified")))
      testClass = getClassPathClazz
      _ <- check(testClass != null, configurationException(TestingSupportBundle.message("test.config.test.class.not.found.in.module", testClassPath, getModule.getName)))
      ...
      _ <- check(config.isValidSuite(testClass), {
        ...
        val message = if (ScalaPsiUtil.isInheritorDeep(testClass, suiteClass)) {
          TestingSupportBundle.message("test.config.no.suite.class.is.found.for.class.in.module", testClassPath, getModule.getName)
        } else {
          TestingSupportBundle.message("test.config.class.is.not.inheritor.of.suite.trait", testClassPath)
        }
