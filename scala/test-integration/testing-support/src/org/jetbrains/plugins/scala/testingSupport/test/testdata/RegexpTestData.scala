...
class RegexpTestData(config: AbstractTestRunConfiguration) extends TestConfigurationData(config) {
  ...
  override def checkSuiteAndTestName: CheckResult =
    for {
      _ <- checkModule
      _ <- checkRegexps(
        (_, p) => configurationException(TestingSupportBundle.message("test.config.failed.to.compile.pattern", p)),
        configurationException(TestingSupportBundle.message("test.config.no.patterns.detected"))
      )
    } yield ()
  ...
  override def getTestMap: Map[String, Set[String]] = {
    ...
    if (isDumb) {
      if (testsBuf.isEmpty)
        throw executionException(TestingSupportBundle.message("test.config.cant.run.while.indexing.no.class.names.memorized.from.previous.iterations"))
    ...
    def getCondition(patternString: String): String => Boolean = {
      try {
        ...
        case e: PatternSyntaxException =>
          throw executionException(TestingSupportBundle.message("test.config.failed.to.compile.pattern", patternString), e)
      }
    }
