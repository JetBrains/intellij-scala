...
/**
 * If you wish to run this test in the IDE, right-click on the class name `CompilerPluginTest_2_12`.
 ...
 *   1. In the "Build and run" section, make sure the module `scalaCommunity.compiler-plugin-2_12.test` module
...
private object CompilerPluginTest_2_12 {
  ...
  private def deleteRecursively(path: Path): Unit = {
    if (Files.isDirectory(path))
      using(Files.list(path))(_.forEach(deleteRecursively))
    Files.delete(path)
  }
