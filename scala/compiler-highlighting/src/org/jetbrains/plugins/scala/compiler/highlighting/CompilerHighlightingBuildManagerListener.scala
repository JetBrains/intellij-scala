...
private final class CompilerHighlightingBuildManagerListener extends BuildManagerListener {
  ...
  override def buildFinished(project: Project, sessionId: UUID, isAutomake: Boolean): Unit = {
    ...
    Tracing(project).mapAndEnd(sessionId) {
      case session: BuildManagerSessionPhaseEvent =>
        session.compilationIds.foreach { compilationId =>
          Tracing(project).mapAndEnd(compilationId)(e => Some(e.closed()))
        }
        Some(session)
    ...
    executeOnBackgroundThreadInNotDisposed(project) {
      if (CompilerLockService.instance(project).isReady &&
        ...
        Tracing(project).instant(HighlightingTriggerPhaseEvent(requestId, "build finished",
          traceSessionId ))
        val dispatched =
          CompilerHighlightingService.get(project).triggerDocumentCompilationInAllOpenEditors(None, "On Build Finish", requestId)
        ...
        if (!dispatched) {
          Tracing(project).instant(EndEvent(requestId, "no document compilation scheduled after build"))
        }
        ...
        Tracing(project).instant(EndEvent(traceSessionId, "build finished"))
      } else {
        Tracing(project).instant(EndEvent(traceSessionId, "Build canceled"))
      }
    }
  }
}
