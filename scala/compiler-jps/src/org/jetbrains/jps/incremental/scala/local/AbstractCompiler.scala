...
abstract class AbstractCompiler extends Compiler {
  ...
  class ClientProgress(client: Client, sourcesCount: Int)
    ...
    private var progressLastTimeChecked = System.currentTimeMillis()
    private final val progressThreshold = 1000L
    ...
    override def advance(current: Int, total: Int, prevPhase: String, nextPhase: String): Boolean = {
      ...
      // An optimization. Notify client about progress rarely.
      if (current == 0 || current == total || currentTime - progressLastTimeChecked > progressThreshold) {
        progressLastTimeChecked = currentTime
        // Text is empty because it is generated by the client
