...
object DocumentCompilationArguments {
  ...
  @deprecated(message = "Use serialize(DocumentCompilationArguments, PathTranslator). Kept for preserving binary compatibility.", since = "2026.1")
  @Deprecated
  @ApiStatus.ScheduledForRemoval(inVersion = "2026.2")
  def serialize(arguments: DocumentCompilationArguments): Seq[String] = serialize(arguments, NioPathTranslator)
