...
class ScalaMatchingVisitor(globalVisitor: GlobalMatchingVisitor) extends ScalaElementVisitor {
  ...
  override def visitTypeElement(teI: ScTypeElement): Unit = {
    ...
    def parameterizedMatch = {
      val patternArgs = te match {
        case patternDef: ScParameterizedTypeElement => Some(patternDef.typeArgList.typeArgsWithNamed)
        case _ => None
      }
      val matchArgs = other match {
        case otherDef: ScParameterizedTypeElement => Some(otherDef.typeArgList.typeArgsWithNamed)
        case _ => None
      }
  ...
  override def visitParameterizedTypeElement(parameterized: ScParameterizedTypeElement): Unit = {
    ...
    def parametersMatch = matchSequentially(parameterized.typeArgList.typeArgsWithNamed, other.typeArgList.typeArgsWithNamed)
