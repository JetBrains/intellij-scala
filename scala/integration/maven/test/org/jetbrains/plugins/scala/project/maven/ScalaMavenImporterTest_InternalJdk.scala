...
class ScalaMavenImporterTest_InternalJdk extends ScalaMavenImporterTest(None):

  @Test
  def resolveReplClasspath_Scala37(using TestInfo): Unit = withProjectJdk:
  ...
  @Test
  def resolveReplClasspath_Scala38(using TestInfo): Unit = withProjectJdk:
    runImportingTest(project(getProject.getName))
