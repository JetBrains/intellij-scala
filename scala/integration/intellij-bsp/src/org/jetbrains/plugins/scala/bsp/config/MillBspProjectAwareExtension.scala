...
class MillBspProjectAwareExtension extends BspProjectAwareExtension {
  override def getEligibleConfigFileExtensions: util.List[String] =
    MillBspPluginConstants.SUPPORTED_CONFIG_FILE_EXTENSIONS.asJava

  override def getEligibleConfigFileNames: util.List[String] =
    MillBspPluginConstants.SUPPORTED_CONFIG_FILE_NAMES.asJava

  override def getProjectId(virtualFile: VirtualFile): ExternalSystemProjectId =
    new ExternalSystemProjectId(MillBspPluginConstants.SYSTEM_ID, virtualFile.getPath)
}