...
private[references] object ClassfileParser {
  private implicit class RichScalaSigSymbol(private val sym: Symbol) extends AnyVal {
  ...
  private class ScalaSigVisitor(file: String) extends ClassVisitor(Opcodes.ASM6) {
  ...
  private def fqnFromInternalName(internal: String): String  = pattern.matcher(internal).replaceAll(".")
  private def isFunExprClassname(name:      String): Boolean = name.contains("$anonfun$")

  private trait ReferenceCollector {
    def handleMemberRef(ref:   MemberReference): Unit
    def handleSAMRef(ref:      FunExprInheritor): Unit
    def handleLineNumber(line: Int): Unit
  }

  private class ParsingVisitor(synthetics: Set[String]) extends ClassVisitor(ASM6) {
    private var internalName: String                            = uninitialized
    private var className: String                               = uninitialized
    private var isAnon: Boolean                                  = false
    private val superNames: mutable.Builder[String, Set[String]] = Set.newBuilder[String]
    private val innerRefs: ju.List[MemberReference]              = new ju.ArrayList[MemberReference]()
    private val funExprs: ju.List[FunExprInheritor]              = new ju.ArrayList[FunExprInheritor]()
    private var earliestSeen: Option[Int]                        = None

    private val methodVisitor = new MethodVisitor(ASM6) with ReferenceInMethodCollector {
    ...
    /**
      ...
      */
    private def isStaticForwarder(access: Int, name: String): Boolean =
      isSet(access, ACC_STATIC) &&
        name != "<clinit>" &&
        name != "$init$" &&
        !name.contains("$anonfun$")

    private def isSynthetic(name: String): Boolean = {
  ...
  private trait ReferenceInMethodCollector extends MethodVisitor with ReferenceCollector {

    private var currentLineNumber = -1

    private def handleMemberRef(
      owner:   String,
      name:    String
    ...
    override def visitFieldInsn(opcode: Int, owner: String, name: String, desc: String): Unit =
      if (currentLineNumber != -1) handleMemberRef(owner, name)(FieldReference.apply)
  }
}
