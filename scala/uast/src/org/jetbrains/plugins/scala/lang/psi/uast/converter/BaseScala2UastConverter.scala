...
/**
  ...
  * clearer.
  */
object BaseScala2UastConverter extends Scala2UastConverter {
  ...
  private def convertToFree[U <: UElement: ClassTag: NotNothing](
    ...
    val elementOpt: Option[Free[UElement]] =
      Option((sourcePsi match {
        ...
        case e: ScCaseClause =>
          if (insideCatchBlock(e)) new ScUCatchExpression(e, _)
          else new ScUCaseClause(e, _)
        case e: ScBlock if e.getParent.isInstanceOf[ScCaseClause] =>
          e.getParent match {
            case clause: ScCaseClause if !insideCatchBlock(clause) =>
...
object ConverterUtils {
  //noinspection ScalaUnusedSymbol
  def insideCatchBlock(c: ScCaseClause): Boolean =
    (for {
