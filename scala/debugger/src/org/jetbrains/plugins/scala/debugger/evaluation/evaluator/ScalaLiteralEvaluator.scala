  ...
  override def evaluate(context: EvaluationContextImpl): Value = value match {
    ...
    case _ =>
      ...
      value match {
        ...
        case _ => throw EvaluationException(DebuggerBundle.message("unknown.type.of.literal"))
      }
  }
}
  ...
  def apply(literal: ScLiteral,
    ...
    value match {
      case null if !`type`.isNull =>
        throw EvaluationException(DebuggerBundle.message("literal.has.null.value", literal.getText))
