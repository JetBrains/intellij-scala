...
abstract class Spec2ExampleUnitSpec extends Specs2TestCase {
  ...
  def testUnitSpecAll(): Unit =
    runTestByLocation2(3, 10, fileName,
      assertConfigAndSettings(_, regExpClassName),
      root => {
        assertResultTreeHasExactNamedPath(root, Seq("[root]", "MutableSpec", "'Hello world' should", "contain 11 characters"))
        assertResultTreeHasExactNamedPath(root, Seq("[root]", "MutableSpec", "'Hello world' should", "start with 'Hello'"))
        assertResultTreeHasExactNamedPath(root, Seq("[root]", "MutableSpec", "'Hello world' should", "with 'world'"))
        // TODO: there are 2 equal paths which are run in different contexts, we should test both
        assertResultTreeHasExactNamedPath(root, Seq("[root]", "MutableSpec", "'Hey you' should", "contain 7 characters"))
      })
}
