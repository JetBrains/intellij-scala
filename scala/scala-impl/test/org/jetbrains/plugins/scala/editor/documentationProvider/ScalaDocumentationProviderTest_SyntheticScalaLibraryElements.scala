...
class ScalaDocumentationProviderTest_SyntheticScalaLibraryElements
  ...
  def testSyntheticMethod_IsInstanceOf_InAny(): Unit = {
    ...
    doGenerateDocAtCaretTest(
      s"""<html><head>$CssStyleSection<base href="jar:file:///$ivyHome/cache/org.scala-lang/scala-library/srcs/scala-library-2.13.14-sources.jar!/scala/Any.scala"></head><body><div class="bottom"><icon src="org.jetbrains.plugins.scala.icons.Icons.CLASS"/>&nbsp;<a href="psi_element://scala.Any"><code>scala.Any</code></a></div><div class='definition'><pre><span style="color:#000080;font-weight:bold;">final</span> <span style="color:#000080;font-weight:bold;">def</span> <span style="color:#000000;">isInstanceOf</span>[<span style="color:#20999d;">T0</span>]: <span style=""><a href="psi_element://scala.Boolean"><code>Boolean</code></a></span></pre></div><div class='content'>Test whether the dynamic type of the receiver object has the same erasure as <tt>T0</tt>.<p>Depending on what <tt>T0</tt> is, the test is done in one of the below ways:</p><ul><li><tt>T0</tt> is a non-parameterized class type, e.g. <tt>BigDecimal</tt>: this method returns <tt>true</tt> if the value of the receiver object is a <tt>BigDecimal</tt> or a subtype of <tt>BigDecimal</tt>.</li><li><tt>T0</tt> is a parameterized class type, e.g. <tt>List[Int]</tt>: this method returns <tt>true</tt> if the value of the receiver object is some <tt>List[X]</tt> for any <tt>X</tt>. For example, <tt>List(1, 2, 3).isInstanceOf[List[String]]</tt> will return true.</li><li><tt>T0</tt> is some singleton type <tt>x.type</tt> or literal <tt>x</tt>: this method returns <tt>this.eq(x)</tt>. For example, <tt>x.isInstanceOf[1]</tt> is equivalent to <tt>x.eq(1)</tt></li><li><tt>T0</tt> is an intersection <tt>X with Y</tt> or <tt>X &amp; Y: this method is equivalent to </tt>x.isInstanceOf[X] && x.isInstanceOf[Y]<tt></tt></li><li><tt>T0</tt> is a union <tt>X | Y</tt>: this method is equivalent to <tt>x.isInstanceOf[X] || x.isInstanceOf[Y]</tt></li><li><tt>T0</tt> is a type parameter or an abstract type member: this method is equivalent to <tt>isInstanceOf[U]</tt> where <tt>U</tt> is <tt>T0</tt>'s upper bound, <tt>Any</tt> if <tt>T0</tt> is unbounded. For example, <tt>x.isInstanceOf[A]</tt> where <tt>A</tt> is an unbounded type parameter will return true for any value of <tt>x</tt>.</li></ul><p>This is exactly equivalent to the type pattern <tt>_: T0</tt></p></div><table class='sections'><tr><td valign='top' class='section'><p>Returns:</td><td valign='top'><tt>true</tt> if the receiver object is an instance of erasure of type <tt>T0</tt>; <tt>false</tt> otherwise.</td><tr><td valign='top' class='section'><p>Note:</td><td valign='top'>due to the unexpectedness of <tt>List(1, 2, 3).isInstanceOf[List[String]]</tt> returning true and <tt>x.isInstanceOf[A]</tt> where <tt>A</tt> is a type parameter or abstract member returning true, these forms issue a warning.</td></table></body></html>""".stripMargin)
  }

  def testSyntheticMethod_HashHash_InAny(): Unit = {
    configureScalaFromFileText(s"""val x: Any = 0 ; x.##$CARET""")
    doGenerateDocAtCaretTest(
      s"""<html><head>$CssStyleSection<base href="jar:file:///$ivyHome/cache/org.scala-lang/scala-library/srcs/scala-library-2.13.14-sources.jar!/scala/Any.scala"></head><body><div class="bottom"><icon src="org.jetbrains.plugins.scala.icons.Icons.CLASS"/>&nbsp;<a href="psi_element://scala.Any"><code>scala.Any</code></a></div><div class='definition'><pre><span style="color:#000080;font-weight:bold;">final</span> <span style="color:#000080;font-weight:bold;">def</span> <span style="color:#000000;">##</span>: <span style=""><a href="psi_element://scala.Int"><code>Int</code></a></span></pre></div><div class='content'>Equivalent to <tt>x.hashCode</tt> except for boxed numeric types and <tt>null</tt>. For numerics, it returns a hash value which is consistent with value equality: if two value type instances compare as true, then ## will produce the same hash value for each of them. For <tt>null</tt> returns a hashcode where <tt>null.hashCode</tt> throws a <tt>NullPointerException</tt>.</div><table class='sections'><tr><td valign='top' class='section'><p>Returns:</td><td valign='top'>a hash value consistent with ==</td></table></body></html>""".stripMargin)
  }
