...
import org.jetbrains.plugin.scala.compilerReferences.{ChunkBuildData, Codec}
...
class ChunkBuildDataCodecTest {
  ...
  def testPrimitive(): Unit =
    shouldRoundtrip(42, false, true, "foobar") 
  ...
  def testBuildDataComplex(): Unit = shouldRoundtrip {
    ...
    ChunkBuildData(classes, sources, modules)
  }
}
