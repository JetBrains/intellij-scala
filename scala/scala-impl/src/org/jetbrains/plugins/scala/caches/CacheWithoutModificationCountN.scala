...
private class CacheWithoutModificationCountN[T <: Product, R](id: String, name: String, wrapper: ValueWrapper[R], cleanupScheduler: CleanupScheduler) {
  // Code generated by @CachedWithoutModificationCount macro annotation, method with parameters

  private[this] val map = new ConcurrentHashMap[T, wrapper.Reference]()
  ...
  def apply(key: T)(f: => R): R = {
    val reference = {
      ...
      if (fromMap != null) fromMap else null.asInstanceOf[wrapper.Reference]
    }
