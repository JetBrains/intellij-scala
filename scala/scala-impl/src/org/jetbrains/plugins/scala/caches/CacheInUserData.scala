...
object CacheInUserData {
  ...
  def cacheInUserData0[E: ProjectUserDataHolder, R >: Null](id: String, name: String, dataHolder: E, dependency: => AnyRef, f: () => R): R = {
    val tracer = Tracer(id, name)
    tracer.invocation()
    val key = CachesUtil.getOrCreateKey[CachesUtil.CachedRef[R]](id)
    val holder = CachesUtil.getOrCreateCachedRef[dataHolder.type, R](dataHolder, key, id, name, () => dependency) // TODO (psiElement)
    ...
    if (stackStamp.mayCacheNow()) {
      val race = {
        holder.compareAndSet(null, result);
        holder.get()
      }
  ...
  def cacheInUserDataN[E: ProjectUserDataHolder, T <: Product, R](id: String, name: String, dataHolder: E, dependency: => AnyRef, v: T, f: T => R): R = {
    val tracer = Tracer(id, name)
    tracer.invocation()
    val key = CachesUtil.getOrCreateKey[CachesUtil.CachedMap[T, R]](id)
    val holder = CachesUtil.getOrCreateCachedMap[dataHolder.type, T, R](dataHolder, key, id, name, () => dependency) // TODO (psiElement)
