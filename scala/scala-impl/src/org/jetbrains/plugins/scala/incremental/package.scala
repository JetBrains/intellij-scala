...
package object incremental {
  ...
  private val combineDirtyScopesMethod = {
    val method = classOf[FileStatusMap].getDeclaredMethod("markFileScopeDirty", classOf[Document], classOf[TextRange], classOf[Object])
  ...
  private[incremental] implicit class DaemonCodeAnalyzerExt(private val daemon: DaemonCodeAnalyzer) extends AnyVal {
    // com.intellij.codeInsight.daemon.impl.FileStatusMap.markFileScopeDirty
