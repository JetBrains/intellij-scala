...
class WorksheetPsiGlue(store: mutable.ListBuffer[QueuedPsi]) {
  ...
  def processPsi(psi: PsiElement): Unit = {
    ...
    (psi, store.lastOption) match {
      case (clazz: ScClass, Some(SingleQueuedPsi(obj: ScObject))) => processInner(clazz, obj, isClassFirst = false)
      case (traid: ScTrait, Some(SingleQueuedPsi(obj: ScObject))) => processInner(traid, obj, isClassFirst = false)
      case (obj: ScObject, Some(SingleQueuedPsi(clazz: ScClass))) => processInner(clazz, obj, isClassFirst = true)
      case (obj: ScObject, Some(SingleQueuedPsi(traid: ScTrait))) => processInner(traid, obj, isClassFirst = true)
