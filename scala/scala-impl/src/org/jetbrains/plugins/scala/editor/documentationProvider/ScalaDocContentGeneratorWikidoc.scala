 ...
 */
private class ScalaDocContentGeneratorWikidoc(
  originalComment: ScDocComment,
  macroFinder: MacroFinder,
) extends ScalaDocContentGenerator {
  ...
  override def appendTagDescriptionText(
    ...
    var isFirst = true
    descriptionParts.iterator.foreach { part =>
  ...
  override def appendDescriptionParts(
    buffer: StringBuilder,
    comment: ScDocComment,
  ): Boolean = {
    var isFirst = true
    comment.descriptionParts.iterator.foreach { part =>
      visitDescriptionPartNode(buffer, part, isFirst)
      isFirst = false
    }
    !isFirst
  ...
  private def visitDocList(buffer: StringBuilder, list: ScDocList): Unit = {
    ...
    import org.jetbrains.plugins.scala.editor.documentationProvider.ScalaDocContentGeneratorWikidoc.DocListType._
  ...
  private def isMarkupInner(child: PsiElement): Boolean =
    child match {
      case _: LeafPsiElement => !child.elementType.is[ScalaDocSyntaxElementType]
      case _                 => true
    }
...
object ScalaDocContentGeneratorWikidoc {
