...
/**
 * Utility to scrape all types that escape from public members of a given type definition. The current use
 ...
 *
 * [[getEscapingTypes]] is the main and only entrypoint.
 */
private[declarationRedundancy] object TypeDefEscaping {

  /**
   * If any of the scraped [[ScType]] instances is parameterized, this method will destructure it into a list of
  ...
  /**
   * Recursively scrape all types that escape from public members of `typeDef`.
   *
   * The result is a list of tuples, associating a public member with a list of the types that escape through it.
   *
   ...
   * escape through members of `Bar`'s direct parent, which is `Foo`. Naturally this includes `Bar`, so when you
   * iterate through `getEscapingTypes`'s results, you have to skip `Bar`.
   *
   * The reason `getEscapingTypes` caches and returns all public members of `Foo` and not just `Bar`'s siblings, is so
  ...
  def getEscapingTypes(typeDef: ScTypeDefinition): Seq[(ScMember, Seq[ScType])] = {
    ...
    typeDef.members.collect { member =>
      ...
      member match {
        case typeDef: ScTypeDefinition if !isPrivate(typeDef) =>
          ...
          val childTypes = (typeDef +: typeDef.baseCompanion.toSeq).flatMap(getEscapingTypes)

          (typeDef -> destructureAndFilter(types)) +: childTypes
        ...
        case typeAlias: ScTypeAliasDefinition if !isPrivate(typeAlias) =>
          ...
          Seq(typeAlias -> destructureAndFilter(types))
        ...
        case primaryConstructor: ScPrimaryConstructor =>
          ...
          Seq(primaryConstructor -> destructureAndFilter(types))
        ...
        case function: ScFunction if !isPrivate(function) =>
          ...
          Seq(function -> destructureAndFilter(types))

        case typeable: Typeable if !isPrivate(typeable) =>
          val types = typeable.`type`().toSeq
          Seq(typeable -> destructureAndFilter(types))
