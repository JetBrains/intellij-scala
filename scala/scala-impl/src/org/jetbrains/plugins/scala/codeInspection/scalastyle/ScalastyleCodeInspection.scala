...
object ScalastyleCodeInspection {
  ...
  private object locations {
    val possibleSrcConfigFileNames = Seq("scalastyle_config.xml", "scalastyle-config.xml")
    val possibleTestConfigFileNames = Seq("scalastyle_test_config.xml", "scalastyle-test-config.xml")
    val possibleLocations = Seq(".idea", "project")

    def findSrcConfigFile(dir: VirtualFile): Option[VirtualFile] = possibleSrcConfigFileNames.flatMap(name => Option(dir.findChild(name))).headOption
    def findTestConfigFile(dir: VirtualFile): Option[VirtualFile] =
      (possibleTestConfigFileNames ++ possibleSrcConfigFileNames).flatMap(name => Option(dir.findChild(name))).headOption

    def findIn(project: Project, findConfigFile: VirtualFile => Iterable[VirtualFile]): Option[VirtualFile] = {
    ...
    def findInSrc(project: Project): Option[VirtualFile] = {
      findIn(project, findSrcConfigFile)
    }

    def findInTest(project: Project): Option[VirtualFile] = {
      findIn(project, findTestConfigFile)
    }
  }
  ...
  def configuration(project: Project): Option[ScalastyleConfiguration] = {
    locations.findInSrc(project).flatMap(latest)
  }

  def testConfiguration(project: Project): Option[ScalastyleConfiguration] = {
    locations.findInTest(project).flatMap(latest)
  }
...
class ScalastyleCodeInspection extends LocalInspectionTool {
  ...
  override def checkFile(file: PsiFile, manager: InspectionManager, isOnTheFly: Boolean): Array[ProblemDescriptor] = {
    def withConfiguration(configuration: Option[ScalastyleConfiguration],
                          f: ScalastyleConfiguration => Iterable[ProblemDescriptor]): Array[ProblemDescriptor] = {
      configuration.map(c => f(c).toArray).getOrElse(Array.empty)
    }


    def findProblems: ScalastyleConfiguration => List[ProblemDescriptor] = {
      configuration =>
    ...
    else if (TestSourcesFilter.isTestSources(file.getVirtualFile, file.getProject)) {
      withConfiguration(ScalastyleCodeInspection.testConfiguration(file.getProject), findProblems)
    } else {
      withConfiguration(ScalastyleCodeInspection.configuration(file.getProject), findProblems)
    }
