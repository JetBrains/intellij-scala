...
import org.jetbrains.plugins.scala.lang.psi.types.{ConformanceContext, ScCompoundType, ScType, ScTypeExt}
...
class ScalaGotoTypeDeclarationProvider extends TypeDeclarationProvider {
  override def getSymbolTypeDeclarations(symbol: PsiElement): Array[PsiElement] = {
    implicit val context: ConformanceContext = ConformanceContext(symbol)
