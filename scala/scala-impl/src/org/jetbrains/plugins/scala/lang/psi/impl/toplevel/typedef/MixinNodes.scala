...
import org.jetbrains.plugins.scala.lang.psi.api.toplevel.typedef.{MixinNodesBase, ScObject, ScTemplateDefinition, ScTrait, ScTypeDefinition}
...
abstract class MixinNodes[T <: Signature](signatureCollector: SignatureProcessor[T]) extends MixinNodesBase[T] {
...
object MixinNodes {
  val currentlyProcessedSigs: RichThreadLocal[JMap[PsiClass, MixinNodesBase.Map[TermSignature]]] =
    new RichThreadLocal(new JMap)

  def withSignaturesFor[T](cls: PsiClass, sigs: MixinNodesBase.Map[TermSignature])(f: =>T): T = try {
  ...
  class MapImpl[T <: Signature] extends MixinNodesBase.Map[T] {
  ...
  def emptyMap[T <: Signature]: MixinNodesBase.Map[T] = new MixinNodes.MapImpl[T]
