    ...
    getEffectiveInvokedExpr.getNonValueType() match {
      case Right(scType) =>
        ...
        checkApplication(nonValueType, invokedResolveResult) match {
          ...
          case _ =>
            val stripTypeArgs = invokedResolveResult.exists {
            ...
            val applyOrUpdateCandidates = this.tryResolveApplyMethod(this, nonValueType, isShape = false, stripTypeArgs = stripTypeArgs)
            applyOrUpdateCandidates match {
              case Array(srr) =>
                val processedType = this.updateGenericType(nonValueType, srr)
              ...
              case _ =>
                val problems = applyOrUpdateCandidates.flatMap(_.problems)
