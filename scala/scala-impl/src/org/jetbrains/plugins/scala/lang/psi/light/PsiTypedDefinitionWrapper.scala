...
class PsiTypedDefinitionWrapper(val delegate: ScTypedDefinition, isStatic: Boolean, isInterface: Boolean,
  ...
  override def findSuperMethods(): Array[PsiMethod] = {
    ...
    def wrap(superSig: Signature): Option[PsiMethod] = superSig.namedElement match {
      case f: ScFunction =>
    ...
    val signature =
      if (isSetter(role)) Signature.setter(name, delegate)
      else Signature.withoutParams(name, ScSubstitutor.empty, delegate)
