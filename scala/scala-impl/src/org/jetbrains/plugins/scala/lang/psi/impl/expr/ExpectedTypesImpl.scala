...
//          else                       mapResolves(ref.multiResolveScala(false), ref.multiType(None))
          mapResolves(ref.multiResolveScala(false), ref.multiType(None))
...
//          } else {
            val multiType = gen.multiType(None)

            gen.multiResolve
              .map(mapResolves(_, multiType))
              .getOrElse(multiType.map((_, false, false)))
//          }
        case _ => Array((invoked.getNonValueType(None), false, false))
      }
