...
class ExpectedTypesImpl extends ExpectedTypes {
  ...
  private def computeExpectedParamType(
    ...
    /** Resolves leading `using` clauses on the method type via implicit search,
     * constraining type parameters and stripping the resolved clauses.
     *
     * E.g. for `def foo[A](using Foo[A])(fn: A => Int)` with `given Foo[String]`,
     * the input `ScTypePolymorphicType(ScMethodType(using Foo[A])(A => Int), Seq(A))`
     * becomes a type where `A = String` and the using clause is stripped. */
    def applyToLeadingImplicits(tpe: ScType): Option[(ScType, Option[ScTypeElement])] = {
      val (updated, _) = expr.updatedWithImplicitArguments(
        tpe,
        canThrowSCE = false,
        updateDeep  = false,
      ...
      val stripEmptyTypeParams = updated match {
        case ScTypePolymorphicType(res, tparams) if tparams.isEmpty => res
        case _                                                      => updated
      }

      computeExpectedParamType(
        expr,
        Right(stripEmptyTypeParams),
        argExprs,
        idx,
        call = call
      )
    ...
    invokedExprType match {
      ...
      case Right(t @ ScTypePolymorphicType(mt: ScMethodType, _)) if mt.hasUsingKW && !isExplicitUsingArgs =>
        applyToLeadingImplicits(t)
...
object ExpectedTypesImpl {
  ...
  /** Per-expression known expected types, set by [[Compatibility]] during overload resolution.
    *
    * Consider:
    * {{{
    * def foo(f: Int => String): String = ???
    * def foo(f: Int => Int): Int = ???
    * foo(x => x + 1)
    * }}}
    *
    * Without this mechanism, typing `x => x + 1` triggers a cycle:
    * {{{
    * (x => x + 1).type()
    *   → expectedType() → expectedTypesForArg(foo(...))
    *     → resolve foo → Compatibility checks (x => x + 1) against each overload
    *       → needs (x => x + 1).type() → expectedType() → ... ∞
    * }}}
    *
    * Previously, this cycle was broken by using shapeResolve in `expectedTypesForArg`, which avoids
    * full argument typing by assigning `Nothing` to all non-function args and `(Any, ..., Any) => Nothing`
    * to function literals.
   * However, shapeResolve is too imprecise for Scala 3 features:
    *  - [[https://docs.scala-lang.org/scala3/reference/changed-features/overload-resolution.html Overload resolution with function parameter types]]:
    *    non-function arguments must be fully typed to narrow overloaded candidates before
    *    function literal parameter types can be inferred.
    *  - Mixed using clauses (e.g. `def foo(x: Int)(using Foo[A])(a: A)`): expected type of `a`
    *    depends on implicit resolution of `Foo[A]`, which requires full typing, not shapes.
    *
    * This map provides a more precise alternative: when [[Compatibility.processParamConformance]]
    * checks a function literal against overloaded alternatives, it stores the expected parameter types
    * from all alternatives via [[withPropagatedExpectedTypes]] BEFORE computing the argument's type.
    * Then [[ScExpression.Ext.expectedTypesEx]] reads them via [[propagatedExpectedTypesFor]] and
    * returns immediately, never reaching `expectedTypesForArg()` and avoiding the recursion.
    *
    * Thread-local to avoid cross-thread races (IntelliJ runs highlighting on multiple threads). */
  import java.{util => ju}
  private case class PropagatedExpectedType(
    parameterTypes: Array[ParameterType],
    canCache:       Boolean
  )

  private val propagatedExpectedTypes =
    new UnloadableThreadLocal[ju.Map[ScExpression, PropagatedExpectedType]](new util.HashMap[ScExpression, PropagatedExpectedType]())

  /** Returns the known expected types for `expr` if set by an enclosing [[withPropagatedExpectedTypes]] scope. */
  def propagatedExpectedTypesFor(expr: ScExpression): Option[Array[ParameterType]] = {
    val cache = propagatedExpectedTypes.value

    if (cache.containsKey(expr)) cache.get(expr).parameterTypes.toOption
    else                         None
  }

  /** Executes `fn` with `pts` (expected parameter types from all overloaded alternatives)
    * stored for `expr`. [[propagatedExpectedTypesFor]] and [[canCacheTypeFor]] will see these
    * during the execution of `fn`. Cleaned up in `finally`. */
  def withPropagatedExpectedTypes[T](
    expr:     ScExpression,
    pts:      Array[ParameterType],
    canCache: Boolean
  )(fn: =>T): T =
    try {
      propagatedExpectedTypes.value.put(expr, PropagatedExpectedType(pts, canCache))
      fn
    } finally {
      propagatedExpectedTypes.value.remove(expr)
    }

  /** Whether permanent caching is allowed for computations on `expr`.
    * Returns `false` when `expr` is (or is a child of) an expression with `canCache = false`
    * in the known expected types map, because such computations transitively depend on
    * transient expected types from overloaded alternatives and must not be permanently cached
    * with a key that doesn't include them.
    * Used by [[org.jetbrains.plugins.scala.caches.CacheWithRecursionGuard]]. */
  def canCacheTypeFor(expr: ScExpression): Boolean = {
    val cache = propagatedExpectedTypes.value

    if (cache.containsKey(expr)) cache.get(expr).canCache
    else
      cache.entrySet().stream().noneMatch { entry =>
        !entry.getValue.canCache && {
          val cachedExpr = entry.getKey
          PsiTreeUtil.isContextAncestor(cachedExpr, expr, false)
        }
      }
  }
}
