...
trait ScalaBounds extends api.Bounds {
  ...
  private val scala3 = Scala3Bounds(projectContext.project)

  override def glb(first: ScType, second: ScType, checkWeak: Boolean)(implicit ctx: Context): ScType =
    if (ctx.isScala3) scala3.glb(first, second, checkWeak)
    else                    scala2.glb(first, second, checkWeak)

  override def lub(first: ScType, second: ScType, checkWeak: Boolean)(implicit ctx: Context): ScType =
