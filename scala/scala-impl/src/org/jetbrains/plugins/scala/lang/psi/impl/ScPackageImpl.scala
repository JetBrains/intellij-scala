...
final class ScPackageImpl private(val pack: PsiPackage) extends PsiPackageImpl(
  ...
  override def findPackageObject(scope: GlobalSearchScope): Option[ScObject] = _findPackageObject(scope)

  private val _findPackageObject = cachedInUserData("ScPackageImpl.findPackageObject", this, ScalaPsiManager.instance(getProject).TopLevelModificationTracker, (scope: GlobalSearchScope) => {
