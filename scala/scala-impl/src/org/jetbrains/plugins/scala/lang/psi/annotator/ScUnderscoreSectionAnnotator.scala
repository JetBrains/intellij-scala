...
trait ScUnderscoreSectionAnnotator extends Annotatable { self: ScUnderscoreSection =>

  override def annotate(holder: AnnotationHolder, typeAware: Boolean): Unit = {
    super.annotate(holder, typeAware)

    ScUnderscoreSectionAnnotator.checkUnboundUnderscore(this, holder)
    // TODO (otherwise there's no type conformance check)
    // super.visitUnderscoreExpression  }
  }
}

private object ScUnderscoreSectionAnnotator {
