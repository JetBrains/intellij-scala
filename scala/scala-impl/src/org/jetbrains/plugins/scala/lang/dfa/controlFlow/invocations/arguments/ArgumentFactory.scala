...
object ArgumentFactory {
  ...
  def buildArgumentsInEvaluationOrder(invocation: MethodInvocation, isTupled: Boolean): Seq[Argument] = {
    val (matchedParams, maybeVarargArgument) = partitionNormalAndVarargArgs(invocation)

    if (isTupled) List(buildTupledArgument(invocation))
    else matchedParams
  ...
  private def partitionNormalAndVarargArgs(invocation: MethodInvocation): (Seq[(ScExpression, Parameter)], Option[Argument]) = {
