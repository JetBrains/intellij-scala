...
class ScStableCodeReferenceImpl(node: ASTNode) extends ScReferenceImpl(node) with ScStableCodeReference {
  ...
  override def multiResolveScala(incomplete: Boolean): Array[ScalaResolveResult] = synchronizedOn(this) {
    cachedWithRecursionGuard("multiResolveScala", this, ScalaResolveResult.EMPTY_ARRAY, BlockModificationTracker(this), Tuple1(incomplete)) {
