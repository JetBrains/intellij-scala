...
class ScParameterClauseImpl private(stub: ScParamClauseStub, node: ASTNode)
  ...
  override def isGiven: Boolean = {
    def hasGivenKeyword =
      getFirstChild.getNode.getElementType == ScalaTokenType.Given

    // check if previous clauses were given.
    // In case the user has forgotten the given keyword,
    // this should still be handled as a given clause,
    def prevClauseIsGiven =
      getPrevSibling.asOptionOf[ScParameterClause].exists(_.isGiven)

    byStubOrPsi(_.isGiven)(hasGivenKeyword || prevClauseIsGiven)
  }
