...
class MyScaladocParsing(private val psiBuilder: PsiBuilder) extends ScalaDocElementTypes {
  ...
  private def parseTag(implicit builder: PsiBuilder): Boolean = {
    ...
    if (!isEndOfComment)
      ...
      scaladocError(builder, ScalaBundle.message("unexpected.end.of.tag.body"))

    if (isInInlinedTag) {
      scaladocError(builder, ScalaBundle.message("inline.tag"))
      tagName match {
      ...
      tagName match {
        ...
        case PARAM_TAG | TYPE_PARAM_TAG | DEFINE_TAG =>
          if (!builder.lookAhead(builder.getTokenType, DOC_TAG_VALUE_TOKEN))
            scaladocError(builder, ScalaBundle.message("missing.tag.param"))
