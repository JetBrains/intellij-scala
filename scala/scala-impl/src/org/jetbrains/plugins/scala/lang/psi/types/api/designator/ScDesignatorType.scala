...
import org.jetbrains.plugins.scala.lang.psi.types.{AliasType, ConstraintSystem, ConstraintsResult, ConformanceContext, LeafType, ScType, ScalaTypeVisitor}
...
final case class ScDesignatorType(override val element: PsiNamedElement) extends DesignatorOwner with LeafType {
  ...
  override protected def calculateAliasType(implicit context: ConformanceContext): Option[AliasType] = calculateAliasTypeAux(element, ScSubstitutor.empty)
