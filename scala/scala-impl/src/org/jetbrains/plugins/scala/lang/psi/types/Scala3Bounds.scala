...
final case class Scala3Bounds(project: Project)
  ...
  override protected def lubInner(
     ...
     */
    def mergeIfSuper(lhs: ScType, rhs: ScType): Option[ScType] =
        ...
        rhs match {
          case ScOrType(l, r) =>
            ...
            upperL match {
              ...
              case Some(other) => Option(lub(other, r, checkWeak))
              case None =>
  ...
  private def mergeIfTypeConstructors(
    ...
    if (lhsParams.nonEmpty && lhsParams.length == rhsParams.length) {
      val depth = lubDepth(lhs, rhs)

      val newParams = typeParametersBound(
        lhsParams,
        rhsParams,
        bound.inverse,
        checkWeak,
      ...
      val tpts = newParams.map(TypeParameterType(_))
      ...
      val newLhs = ScParameterizedType(lhs, tpts)
      val newRhs = ScParameterizedType(rhs, tpts)

      ScTypePolymorphicType(original(newLhs, newRhs), newParams)
    } else op(lhs, rhs)
  }
