...
package object libextensions {
  ...
  class ExtensionAlreadyLoadedException(file: Path) extends ExtensionException(s"Extensions jar $file is already loaded")
  class NoManifestInExtensionJarException(file: Path) extends ExtensionException(s"Extensions jar $file has no manifest")
