...
import org.jetbrains.plugins.scala.lang.psi.types.{PhysicalSignature, ScExistentialArgument, ScExistentialType, ScParameterizedType, ScType, Signature}
...
object SAMUtil {
  ...
  implicit class PsiClassToSAMExt(private val cls: PsiClass) extends AnyVal {
    ...
    private[SAMUtil] def singleAbstractMethodWithSubstitutor: Option[(PsiMethod, ScSubstitutor)] = {
      val allSignatures = TypeDefinitionMembers.getSignatures(cls).allFirstSeq().iterator.flatMap(_.nodesIterator.map(_.info))
      val abstractMembers = allSignatures.filter(SignatureStrategy[Signature].isAbstract).toSeq
      abstractMembers match {
        case Seq(PhysicalSignature(m: PsiMethod, subst))
