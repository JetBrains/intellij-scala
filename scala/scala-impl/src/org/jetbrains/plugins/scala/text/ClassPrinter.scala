...
class ClassPrinter(isScala3: Boolean, extendsSeparator: String = " ", withPrivate: Boolean = true, normalize: Boolean = false) {
  ...
  private def textOfBoundsIn(o: ScTypeBoundsOwner): String = {
    val lower = o.lowerTypeTreeHolder.flatMap(_.`type`().toOption).map(textOf(_)).filter(_ != "_root_.scala.Nothing").map(" >: " + _).getOrElse("")
    val upper = o.upperTypeTreeHolder.flatMap(_.`type`().toOption).map(textOf(_)).filter(_ != "_root_.scala.Any").map(" <: " + _).getOrElse("")
    lower + upper
  }
