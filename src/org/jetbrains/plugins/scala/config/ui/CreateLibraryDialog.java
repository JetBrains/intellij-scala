...
public class CreateLibraryDialog extends DialogWrapper {
  ...
  public CreateLibraryDialog(Project project, final String selectedName) {
    super(project, true);
    setModal(true);
    setTitle(ScalaBundle.message("facet.create.lib.title"));
    myInProject.setSelected(true);
    myInProject.setMnemonic(KeyEvent.VK_P);
    myGlobal.setMnemonic(KeyEvent.VK_A);

    myInProject.setText(ScalaBundle.message("facet.create.project.lib", selectedName));
    myGlobal.setText(ScalaBundle.message("facet.create.application.lib", selectedName));
    init();
  }
