  ...
  protected implicit class RichAttributesTree[T <: m.Tree](ptree: T) {
    ...
    def withDenot[P <: PsiElement](elem: Option[P]): T = {
      val ptree1 = ptree match {
        case ptree: m.Name.Anonymous => ptree.copy(denot = denot(elem), sigma = h.Sigma.Naive)
        case ptree: m.Name.Indeterminate => ptree.copy(denot = denot(elem), sigma = h.Sigma.Naive)
        case ptree: m.Term.Name => ptree.copy(denot = denot(elem), sigma = h.Sigma.Naive)
        case ptree: m.Type.Name => ptree.copy(denot = denot(elem), sigma = h.Sigma.Naive)
        ...
        case ptree: m.Ctor.Name => ptree.copy(denot = denot(elem), sigma = h.Sigma.Naive)
        case _ => ???
      }
      ptree
    }
