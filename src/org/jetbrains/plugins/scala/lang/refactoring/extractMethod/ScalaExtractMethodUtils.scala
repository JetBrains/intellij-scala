...
object ScalaExtractMethodUtils {
  def createMethodFromSettings(settings: ScalaExtractMethodSettings)
    ...
    val needReturnTypeResult = needReturnType(settings.nextSibling, settings.visibility)
    val retType =
      if (!needReturnTypeResult) " = "
  ...
  def needReturnType(nextElement: PsiElement, visibility: String): Boolean = {
    var isPrivate = false
    ...
    val ovPolicy = settings.OVERRIDING_METHOD_TYPE_ANNOTATION
    val simplePolicy = settings.SIMPLE_METHOD_TYPE_ANNOTATION

    TypeAnnotationUtil.needTypeAnnotation(
      TypeAnnotationUtil.requirementForMethod(isLocal, isPrivate, isPublic, isProtected, settings),
      ovPolicy,
      simplePolicy,
      isOverride,
      isSimple
  ...
  def previewSignatureText(settings: ScalaExtractMethodSettings): String = {
    ...
    val returnTypeText = if (needReturnType(settings.nextSibling, settings.visibility))
