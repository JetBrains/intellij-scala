...
class ScFileStubImpl(file: ScalaFile,
  ...
  override def getType: IStubFileElementType[Nothing] =
    tokenSets.elementTypes.file.asInstanceOf[IStubFileElementType[Nothing]]

  def getClasses: Array[PsiClass] =
    getChildrenByType(tokenSets.templateDefinitionSet, PsiClass.ARRAY_FACTORY)

  def getFileName: String = StringRef.toString(fileName)
