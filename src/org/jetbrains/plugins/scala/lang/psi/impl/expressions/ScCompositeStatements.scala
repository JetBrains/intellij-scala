  ...
  */
  ...
  case class ScIfStmtImpl(node: ASTNode) extends ScalaExpression(node) with IfElseIndent with CondIndent{
    ...
    def isCondition = (e: PsiElement) => {
      e.isInstanceOf[ScExpr1Impl] &&
      {
        val parent = e.getParent
        var last = for (val child <- parent.getChildren; child.isInstanceOf[ScExpr1Impl]) child
        parent != last
      }
    }
  ...
  case class ScDoStmtImpl(node: ASTNode) extends ScExpr1Impl(node)  with IfElseIndent with CondIndent{
    ...
    def isCondition = (e: PsiElement) => {
      ...
      {
        val parent = e.getParent
        val last = for (val child <- parent.getChildren; child.isInstanceOf[ScExpr1Impl]) child
        parent != last
      }
    }
  ...
  case class ScWhileStmtImpl(node: ASTNode) extends ScExpr1Impl(node) with IfElseIndent with CondIndent{
