...
case class ScExistentialType(quantified : ScType,
  ...
  override def equivInner(r: ScType, uSubst: ScUndefinedSubstitutor, falseUndef: Boolean)
    ...
    quantified match {
      case ParameterizedType(a: ScAbstractType, args) if !falseUndef =>
      ...
      case ParameterizedType(a: UndefinedType, args) if !falseUndef =>
        r match {
          case ParameterizedType(degignator, _) =>
            val name = a.parameterType.nameAndId
            undefinedSubst = undefinedSubst.addLower(name, degignator)
            undefinedSubst = undefinedSubst.addUpper(name, degignator)
            return ScExistentialType(ScParameterizedType(degignator, args), wildcards).equiv(r, undefinedSubst, falseUndef)
          case ScExistentialType(ParameterizedType(designator, _), _) =>
            val name = a.parameterType.nameAndId
            undefinedSubst = undefinedSubst.addLower(name, designator)
            undefinedSubst = undefinedSubst.addUpper(name, designator)
            return ScExistentialType(ScParameterizedType(designator, args), wildcards).equiv(r, undefinedSubst, falseUndef)
...
object ScExistentialType {
  def simpleExistential(lowerBound: ScType, upperBound: ScType, name: String = "_$1", args: List[TypeParameterType] = Nil): ScExistentialType = {
    ScExistentialType(TypeVariable(name), List(ScExistentialArgument(name, args, lowerBound, upperBound)))
  }
}

case class ScExistentialArgument(name: String, args: List[TypeParameterType],
  ...
  def subst(substitutor: ScSubstitutor): ScExistentialArgument = {
    ScExistentialArgument(name, args.map(t => substitutor.subst(t).asInstanceOf[TypeParameterType]),
