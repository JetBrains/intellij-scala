...
import codeInspection.parentheses.UnnecessaryParenthesesUtil
...
class RemoveUnnecessaryParenthesesIntention extends PsiElementBaseIntentionAction{
  ...
  def isAvailable(project: Project, editor: Editor, element: PsiElement): Boolean = {
    Option(PsiTreeUtil.getParentOfType(element, classOf[ScParenthesisedExpr], false)).exists {
      UnnecessaryParenthesesUtil.canBeStripped(_)
    }
  }
  ...
  def invoke(project: Project, editor: Editor, element: PsiElement) {
    Option(PsiTreeUtil.getParentOfType(element, classOf[ScParenthesisedExpr])).map {
      case expr if UnnecessaryParenthesesUtil.canBeStripped(expr) =>
        val newExpr = ScalaPsiElementFactory.createExpressionFromText(UnnecessaryParenthesesUtil.getTextOfStripped(expr), expr.getManager)
        inWriteAction {
