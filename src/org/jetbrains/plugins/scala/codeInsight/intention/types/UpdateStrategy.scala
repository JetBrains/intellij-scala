...
abstract class UpdateStrategy extends Strategy {
  ...
  def addTypeAnnotation(t: ScType, context: PsiElement, anchor: PsiElement, editor: Option[Editor]) {
    ...
    editor match {
      case Some(e) =>
        val added = addActualType()
        val project = context.getProject
        val types: List[ScType] = t match {
          case ScCompoundType(comps, _, _) => t :: comps.toList
          case _ => List(t)
        }
        val supers: List[ScType] = (types ++ types.flatMap(tp => ScalaPsiUtil.allSuperTypes(tp, project))).distinct
        val expression = new Expression {
          val lookupItems: Array[LookupElement] = {
            val superTexts: List[ScTypeText] = supers.map(ScTypeText)
            superTexts.map { typeText =>
              val useCanonicalText: Boolean = superTexts.exists {
